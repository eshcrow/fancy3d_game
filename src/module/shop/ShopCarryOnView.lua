...
function UIShopCarryOn:ShowMyMoney()
	...
	objSwf.moneyName.htmlText = "Hiện tại" .. t_item[s_Money[self.showType]].name ..":"
...
function UIShopCarryOn:SetSlotInfo(UI, info)
	...
	if self.showType == ShopConsts.T_Back then
		...
		if MainPlayerModel.humanDetailInfo.eaBindGold < info:GetPrice() then
			UI.txtMoney.htmlText = string.format(StrConfig.shop801, "Ngân lượng:") .. string.format(StrConfig.shoppingmall001, "#ff0000", info:GetPrice())
		else
			UI.txtMoney.htmlText = string.format(StrConfig.shop801, "Ngân lượng:" .. info:GetPrice())
		end
	...
	if self.showType == ShopConsts.T_Guild and cfg.needLvl > UnionModel.MyUnionInfo.level then
		UI.buynum.htmlText = string.format(StrConfig.shoppingmall001, "#ff0000", "Bang phái đạt tới" .. cfg.needLvl .. "Cấp")
	...
	elseif cfg.dayLimit == 0 then
		...
		if num >= cfg.dayLimit then
			UI.buynum.htmlText = string.format(StrConfig.shop801, "Mỗi ngày:") .. string.format(StrConfig.shoppingmall001, "#ff0000", 0 .. "/" ..cfg.dayLimit)
			UI.btnBuy.disabled = true
			UI.btn.click = function() end
		else
			UI.buynum.htmlText = string.format(StrConfig.shop801, "Mỗi ngày:") .. string.format(StrConfig.shoppingmall001, "#00ff00", (cfg.dayLimit - num) .. "/" ..cfg.dayLimit)
			UI.btnBuy.disabled = false
			UI.btnBuy.click = function() self:OnItemClick(info) end
			UI.btn.click = function() self:OnItemClick(info) end
		end
...
function UIShopCarryOn:OnItemClick(shopVO)
	if shopVO:GetNeedLevel() > MainPlayerModel.humanDetailInfo.eaLevel then
		FloatManager:AddNormal("Người chơi đạt đến cấp" ..shopVO:GetNeedLevel() .. "có thể mua")
		return
	end
