...
-- Máy chủ thông báo:vip经验
function VipController:OnVipExp( msg )
	QuestController:TestTrace( "Máy chủ thông báo:vip Kinh nghiệm" )
	QuestController:TestTrace( msg )

	VipModel:SetVipExp( msg.exp )
end

-- Máy chủ thông báo:vip剩余时间
function VipController:OnVipPeriod( msg )
	QuestController:TestTrace( "Máy chủ thông báo:vip Còn thừa thời gian" )
	QuestController:TestTrace( msg )
...
-- Máy chủ thông báo:vip等级奖励领取状态
function VipController:OnVipLevelRewardState( msg )
	QuestController:TestTrace( "Máy chủ thông báo:vip đẳng cấp ban thưởng" )
	QuestController:TestTrace( msg )
...
-- Máy chủ thông báo:vip周奖励领取状态
function VipController:OnVipWeekRewardState( msg )
	QuestController:TestTrace( "Máy chủ thông báo:vip thưởng tuần" )
	QuestController:TestTrace( msg )
	
	VipModel:SetWeekRewardState( msg.state )
end

-- Máy chủ thông báo:续费(激活)vip结果
function VipController:OnVipRenew( msg )
	QuestController:TestTrace( "Máy chủ thông báo:Gia hạn (kích hoạt) Vip" )
	QuestController:TestTrace( msg )
	...
	if result == 1 then
		FloatManager:AddCenter("Nguyên bảo không đủ")
		return
	end
end

-- Máy chủ thông báo:领取vip等级奖励结果
function VipController:OnVipLevelRewardAccept( msg )
	QuestController:TestTrace( "Máy chủ thông báo:Nhận vip ban thưởng cấp" )
	QuestController:TestTrace( msg )
...
-- Máy chủ thông báo:领取vip每周奖励结果

function VipController:OnVipWeekRewardAccept( msg )
	QuestController:TestTrace( "Máy chủ thông báo:Nhận vip thưởng tuần" )
	QuestController:TestTrace( msg )
...
function VipController:OnVipInitStateResult(msg)
	FTrace(msg, 'vip đăng nhập trả lại')
...
-- Máy chủ thông báo:vip返还信息结果
function VipController:OnVipBackInfo( msg )
	trace( "Máy chủ thông báo:vip Trả về tin tức kết quả" )
	--trace( msg )
...
-- Máy chủ thông báo:Nhận lấy vip Trả về kết quả
function VipController:OnGetVipBack( msg )
	FTrace( msg, "Máy chủ thông báo:Nhận lấy vip Trả về kết quả" )
	local backType = msg.backType
	local result = msg.result
	if result == 0 then
		print(backType,"backType:Nhận lấy vip Trả về kết quả")
...
-- Yêu cầu người chơi:续费(激活)vip
function VipController:ReqRenewVip(vipType)
	QuestController:TestTrace( "Yêu cầu người chơi:Phí gia hạn(Kích hoạt)vip" )
...
-- Yêu cầu người chơi:领取vip等级奖励
function VipController:ReqAcceptVipLevelReward(vipLevel)
	QuestController:TestTrace( "Yêu cầu người chơi:Nhận phần thưởng mức vip" )
...
-- Yêu cầu người chơi:领取vip每周奖励
function VipController:ReqAcceptVipWeekReward()
	QuestController:TestTrace( "Yêu cầu người chơi:Nhận lấy vip Mỗi tuần ban thưởng" )
	
	MsgManager:Send( ReqVipWeekRewardAcceptMsg:new() )
end

-- Yêu cầu người chơi:请求vip返还信息
function VipController:ReqVipBackInfo(backType)
	local msg = ReqVipBackInfoMsg:new()
	msg.backType = backType
	MsgManager:Send(msg)
	FTrace( msg, "Yêu cầu người chơi:thông tin vip" )
end

-- Yêu cầu người chơi:Nhận lấy vip Trả về kết quả
function VipController:ReqGetVipBack(backType)
	local msg = ReqGetVipBackMsg:new()
	msg.backType = backType
	MsgManager:Send(msg)
	FTrace( msg, "Yêu cầu người chơi:Nhận lấy vip Trả về kết quả" )
end
...
function VipController:GetAttName(attname, attValue)
	if attname == 'att' then
		return '<font size = "14" color = "#E59445">Công kích:  </font>'..'<font size = "14" color = "#D5D0C2">'.. attValue ..'</font>'
	elseif attname == 'def' then
		return '<font size = "14" color = "#E59445">Phòng ngự:  </font>'..'<font size = "14" color = "#D5D0C2">'.. attValue ..'</font>'
	elseif attname == 'hp' then
		return '<font size = "14" color = "#E59445">Sinh mệnh:  </font>'..'<font size = "14" color = "#D5D0C2">'.. attValue ..'</font>'
	elseif attname == 'hit' then
		return '<font size = "14" color = "#E59445">Chính xác:  </font>'..'<font size = "14" color = "#D5D0C2">'.. attValue ..'</font>'
	elseif attname == 'dodge' then
		return '<font size = "14" color = "#E59445">Né tránh:  </font>'..'<font size = "14" color = "#D5D0C2">'.. attValue ..'</font>'
	elseif attname == 'critical' then
		return '<font size = "14" color = "#E59445">Bạo kích:  </font>'..'<font size = "14" color = "#D5D0C2">'.. attValue ..'</font>'
	elseif attname == 'defcri' then
		return '<font size = "14" color = "#E59445">Sức bền:  </font>'..'<font size = "14" color = "#D5D0C2">'.. attValue ..'</font>'
	elseif attname == 'cri' then
		return '<font size = "14" color = "#E59445">Bạo kích:  </font>'..'<font size = "14" color = "#D5D0C2">'.. attValue ..'</font>'
	elseif attname == 'crivalue' then
		return '<font size = "14" color = "#E59445">Bạo tổn thương:  </font>'..'<font size = "14" color = "#D5D0C2">'.. attValue ..'</font>'
	elseif attname == 'speed' then
		return '<font size = "14" color = "#E59445">Tốc độ:  </font>'..'<font size = "14" color = "#D5D0C2">'.. attValue ..'</font>'
	end
...
function VipController:BagExtraNumTips(index,vipUseNum)
	...
	return  'Trở thành'..vipName..'vip thêm số lần sử dụng:<font color="#65c47e">'..num..'</font>'
end
