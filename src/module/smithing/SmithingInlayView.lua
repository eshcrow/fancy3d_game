...
function UISmithingInlay:RefreshGems()
	...
	for i=1,#gems do
		...
		if item then
			...
			self.objSwf['txt_curLv' .. i].htmlText = gem.level .. "Cấp"
			if gem.used then
				item.lvBtn.htmlLabel = "Thăng Cấp"
				local cfg = t_gemgroup[gem.id];
			...
			elseif gem.lvLimit > MainPlayerModel.humanDetailInfo.eaLevel then
				if gem.lvLimit > 99 then
					lvLimit = gem.lvLimit .. "Cấp"
				else
					lvLimit = gem.lvLimit .. "Cấp"
				end
			else
				item.lvBtn.htmlLabel = "Kích hoạt"
				canIn = true
			...
			item.tfHint.text = gem.level .. "Cấp"
			...
			if lvLimit ~= "" then
				item.lvBtn.htmlLabel = lvLimit .. "Mở ra"
			elseif gem.level == 9 then
				item.lvBtn.htmlLabel = "Tối đa"
			end
...
function UISmithingInlay:ShowGemPro()
	...
	for i=1,#gems do
		...
		if self.lvBtnRollNum == i and gem.level ~= 9 then
			objSwf['txt_curLv' .. i].htmlText = (gem.level + 1) .. "Cấp" .. "<font color = '#ffffff'>[Xem trước]<font>"
			...
			objSwf['txt_curLv' .. i].htmlText = gem.level .. "Cấp"
			if gem.used then
				local cfg = t_gemgroup[gem.id];
				proValue = proValue + cfg.atr1
