...
function SmithingStarItemView:AskStarUp()
	if not self.currSelect then
		if self.starType == 1 then
			FloatManager:AddNormal("Vui lòng chọn TInh thạch thăng tinh 12 trở xuống!")
		else
			FloatManager:AddNormal("Vui lòng chọn tinh thạch thăng tinh 12~24!")
		end
		return
	end
...
function SmithingStarItemView:OnBodyEquipClick(e)
	...
	if not EquipUtil:IsCanStarUpByItem(item, self.starType) then
		FloatManager:AddNormal("Trang bị vượt quá giới hạn!")
	end
