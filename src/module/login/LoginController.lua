...
function LoginController:ExecuteEnterGame()
	if _sys:isFontExisting("Tahoma") then
		self:DoExecuteEnterGame();
		return;
	end
	UIFontInst:Show();
	local onFinish = function()
		_sys:installFont("resfile/font/font.ttf");
		UIFontInst:Hide();
		self:DoExecuteEnterGame();
	end;
	...
	UILoaderManager:LoadList({"resfile/font/font.ttf"},onFinish,onProgress);
end

function LoginController:DoExecuteEnterGame()

	-- Dev by ThemGai-Design
	-- Login Now
    ...
    if name == "" then
        UIConfirm:Open("Faça o login novamente com o Launcher!");
        return
    end
    local ret = name:find('%w+%a*%d*')
    if ret ~= 1 then
        UIConfirm:Open("Algo errado com sua conta, tente fazer o login novamente!");
        return
    end
	self:Login(name);
	-- Login
...
function LoginController:Login(accountID)
	...
	if _G.gateServerAddress:lead("http") then
		print("Solicitando endereço do servidor");
		...
		_sys:httpGet(url, function(data)
			...
			print("Obtendo o endereço do servidor",data);
...
function LoginController:OnLoginRoleInfo(msg)
	CLoginScene:Create()
	FTrace(msg,'Retornar informações de login do jogador')
...
function LoginController:OnCreateRoleResult(msg)
	...
	if msg.resultCode == 0 then
		...
		if self.isAutoCreate then
			FPrint('Criado automaticamente com sucesso')
...
function LoginController:NoticeMClient()
	...
	if mclienturl and mchecksum then
		UIMClientNotice:Hide();
	end
end

--请求微端登录url
function LoginController:GetMLoginUrl()
	print("Solicitando uma URL de login de micro portas");
...
function LoginController:OnMLoginUrl(msg)
	print("Retornando para a URL de login da Micro-porta",msg.url);
...
function LoginController:Update(dwInterval)
	...
	if GetCurTime() - self.lastBeatSend > 3000 then
		if self.beatState == 0 then
			...
			print("Error:Tempo limite");
			UIChat:ClientText("<font color = '#ff0000'>" .. "！！！Tempo limite！！！" .. "</font>");
			-- if IsPlatform then
				-- ConnManager:close();
				-- self.beatOpen = false;
			-- end
