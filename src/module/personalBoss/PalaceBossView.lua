...
function UIPalaceBoss:OnShowBossList()
	...
	for i=1,#list do
		...
		vo.id 		= cfg.id;
		if toint(cfg.level) > MainPlayerModel.humanDetailInfo.eaLevel then
			vo.lv       = string.format("<font color='#FF0000'>Đẳng cấp khiêu chiến: %s</font>", cfg.level)
		else
			vo.lv       = string.format("<font color='#ffffff'>Đẳng cấp khiêu chiến: %s</font>", cfg.level)
		end
