...
function UIRelicView:AskStarUp()
	...
	if nextCfg.astrict > MainPlayerModel.humanDetailInfo.eaLevel then
		FloatManager:AddNormal("Nhân vật cấp bậc chưa đủ!")
		return
	end
	if MainPlayerModel.humanDetailInfo.eaBindGold < nextCfg.num then
		FloatManager:AddNormal("Ngân lượng không đủ")
		return
	end
...
function UIRelicView:RefreshSelect()
	...
	self.objSwf.txt_desc.htmlText = "Nhắc nhở: Thay đổi" .. StrConfig["role" ..(t_newequip[self.currSelect:GetParam()].part + 244)] .."Sẽ không ảnh hưởng đẳng cấp tinh luyện!"
...
function UIRelicView:ShowRelicInfo()
	...
	if nextCfg then
		...
		self.objSwf.txt_lv.htmlText = "Nhân vật đẳng cấp đạt tới" .. string.format("<font color='%s'>%s</font>", color, nextCfg.astrict) .. "Cấp" --"强化限制：" .. nextCfg.astrict
		color = "#00ff00"
		if MainPlayerModel.humanDetailInfo.eaBindGold < nextCfg.num then
			color = "#FF0000"
			bCanUp = false
		end
		self.objSwf.txt_cost.htmlText = "Tinh luyện tiêu hao:" .. string.format("<font color='%s'>%s</font>", color, getNumShow(nextCfg.num)) .."Ngân lượng"
		if bCanUp then
