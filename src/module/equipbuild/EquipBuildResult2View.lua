...
function UIEquipBuildResultTwo:GetNewSuperAttr()
	...
	-- 		return "";
	-- 	end
	-- 	local defStr = string.format("随机获得%s条Trác tuyệt属性",self.tipsVO.newSuperDefStr);
	-- 	local str = "";
...
function UIEquipBuildResultTwo:GetSuperAttr()
	...
	for i=1,self.tipsVO.superVO.superNum do
		...
		if vo.id == 0 then
			...
			str = str .. self:GetHtmlText("(Không rãnh) nhưng kèm theo ( Thông qua chế tạo-tiến hành kèm theo","#5a5a5a",TipsConsts.Default_Size,false);
			str = str .. "</p></textformat>"; 
		else
