...
function UIBogeyPilluseListView:GetItemData(pillList)
	...
	for k,v in pairs(pillList) do
		...
		if cfg then
			vo.id   = v.id
			vo.text = string.format("Sử dụng <font color = '#00ff00'>%s </font>*%s",cfg.name,v.num)
...
function UIBogeyPilluseListView:ShowFight( )
	local objSwf = self.objSwf
	if not objSwf then return end
	objSwf.text_fight.htmlText = string.format("Lần này tăng lên <font color= '#00ff00' size = '16'>%s</font> Lực chiến",self.fightNum)
end
