	...
	if pos ~= 0 then
		...
		if item then
			...
			if cfg and not cfg.depot then
				FloatManager:AddNormal("Vật phẩm không thể để vào kho")
				return
			end
		end
	end
...
function BagController:OnDiscardItemResult(msg)
	...
	FloatManager:AddCenter("Hủy bỏ các mục, các loại lỗi không được xử lý"..msg.result);
end

--交换物品结果
function BagController:OnSwapItemResult(msg)
	if msg.result ~= 0 then
		FloatManager:AddCenter("Mục trao đổi, các loại lỗi không được xử lý"..msg.result);
		return;
	end
...
--		end
	...
	FloatManager:AddCenter("Sử dụng vật phẩm,loại lỗi không được xử lý"..msg.result);
end
...
function BagController:OnSellItemResult(msg)
	...
	FloatManager:AddCenter("Bán ra vật phẩm,loại lỗi không được xử lý"..msg.result);
end
...
function BagController:OnSplitItemResult(msg)
	...
	FloatManager:AddCenter("Tác vật phẩm,loại lỗi không được xử lý"..msg.result);
end
--开礼包结果
