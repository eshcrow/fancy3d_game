...
function EquipController:ReqBuildAttrScroll(list)
	QuestController:TestTrace("Yêu cầu tạo các thuộc tính bổ sung")
...
function EquipController:OnBuildAttrScroll(msg)
	QuestController:TestTrace("Tạo kết quả thuộc tính bổ sung")
...
function EquipController:ReqEquipGroupLevelUp(equipId, isBind)
	...
	msg.isBind = isBind
	MsgManager:Send(msg)
	QuestController:TestTrace("Yêu cầu: Sáo trang thăng cấp")
end

-- 服务器返回:套装升级
function EquipController:OnEquipGroupLevelUp(msg)
	QuestController:TestTrace("Trả về: Sáo trang thăng cấp")
	QuestController:TestTrace(msg)
...
function EquipController:OnWashLvUpResult(msg)
	if msg.result == 0 then
		...
		FloatManager:AddNormal("Tẩy luyện thất bại")
	end
end
