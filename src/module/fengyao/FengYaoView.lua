...
function UIFengYao:OnStateRefreshRollOver()
	...
	local type = FengYaoUtil:GetIsStateRefresh();
	if type == 1 then
		strcan = "<font color='#00ff00'>(Đã đạt thành)</font>";
	else
		strcan = "<font color='#cc0000'>(Chưa đạt thành)</font>";
	end
...
function UIFengYao:ShowFengYao(bshowmodel)
	...
	objSwf.tffengyaonum.text = j.."Lần";
end
...
function UIFengYao:GetShowCountHanZi(count)
	if count < 10000 then
		...
		if num >= 10000 then
			return toint(num/10000,-1).."Vạn";
		else
			return num;
		end
	elseif count < 100000000 then
		return toint(count/10000,-1).."Vạn";
	else
		return toint(count/100000000,-1).."Ức";
	end
end
