...
function UIOperactivitesAward:UpdateUI()
	...
	objSwf.txtAwardTimes.text = "Hôm nay đã rút thưởng:" ..(index - 1) .."Lần"

	if actVO.isAward == 2 then
		objSwf.awardBtn.disabled = true
		objSwf.txtAwardTimes.text = "Hôm nay đã rút thưởng:" ..index .."Lần"
	else
		objSwf.awardBtn.disabled = false
	end
	...
	if actVO then
		...
		if index == 4 then
			if VipController:GetVipType() < 1 then
				...
				UIConfirm:Open("VIP Bạch Ngân mới có thể nhận thưởng, có muốn mở",func,nil,"Đã mở", nil, nil, nil, nil, rewardList[actVO.param1 + 1].id .. "," .. rewardList[actVO.param1 + 1].count ..",1")
				return
			end
		end
		if index == 5 then
			if VipController:GetVipType() < 2 then
				...
				UIConfirm:Open("VIP Hoàng Kim mới có thể nhận thưởng, có muốn mở",func,nil,"Đã mở", nil, nil, nil, nil, rewardList[actVO.param1 + 1].id .. "," .. rewardList[actVO.param1 + 1].count ..",1")
				return
			end
		end
