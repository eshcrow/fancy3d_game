...
function UIOperactivitesFight:GetRewardClick()
	if GetCurTime() - self.lastTime > OperactivitiesConsts.delayReawrd then
		...
		if state == 1 then
			...
			UIConfirm:Open("Cần tốn "..cost[2] .."Nguyên bảo để mua" .. string.format("<font color = '%s'><u>%sx%s</u></font>", TipsConsts:GetItemQualityColor(itemCfg.quality), itemCfg.name, award.count),goFunc,nil,nil,nil,nil,nil,nil,actVo.reward)
		else
