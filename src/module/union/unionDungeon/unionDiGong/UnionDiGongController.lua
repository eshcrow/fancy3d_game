...
function UnionDiGongController:OnUnionEnterDiGongWarMsg(msg)
	...
	if msg.result == -1 then
		FloatManager:AddNormal("Tiến vào địa cung thất bại");
		return
	end; 
	if msg.result == -2 then 
		--帮派没有进入权限
		FloatManager:AddNormal("Người chơi cấp bậc chưa đủ");
		return 
	end;
	if msg.result == -3 then
		FloatManager:AddNormal("Số người tham dự đã đủ, đợi lần sau");
		return 
	end
	if msg.result == -4 then
		FloatManager:AddNormal("Hiện tại hoạt động không mở");
		return 
	end
	if msg.result == -5 then
		FloatManager:AddNormal("Không cho phép tổ đội");
		return 
	end
	if msg.result == -7 then
		FloatManager:AddNormal("Vượt phục không thể tham gia");
		return 
	end
	if msg.result == -8 then
		FloatManager:AddNormal("Đạo cụ không đủ")
		return;
	end
