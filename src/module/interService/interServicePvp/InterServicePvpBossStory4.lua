...
function UIInterServiceBossStory4:OnBuffItemOver(buffType)
	...
	if statueList and statueList[buffType] then
		if statueList[buffType].status == 1 then
			str = str .. '<font size = "14" color="#2FE00D">Hiện tại đã chiếm lĩnh, chọn tiến về</font>'
		else
			str = str .. '<font size = "14" color="#CC0000">Hiện tại chưa chiếm lĩnh, chọn tiến về</font>'
		end
	end
	str = str .. '</textformat>'
