...
UIXinfaSkillBasic.currSkillLvl = 0;	  ---当前技能等Cấp
--技能列表
...
function UIXinfaSkillBasic:GetSkillListVO(skillId,lvl,skillindex)
	...
	vo.lvlStr = math.floor(math.floor(skillId % 10000000) % 1000 ).."Cấp"..lvl.."trọng";
...
function UIXinfaSkillBasic:ShowRight(skillId,lvl)
	...
	if cfg then
		...
		objSwf.infopanel.tfLvl.htmlText = string.format(StrConfig['magicskill001'],math.floor(math.floor(skillId % 10000000) % 1000),lvl);   ---%s/%strọng
...
function UIXinfaSkillBasic:ShowLearnUpPanel()
	...
	elseif self.currSkillLvl < maxLvl then
		...
		if UIMagicSkillBasic:CanFeedItem(itemList[1].id,itemList[1].num) then   --- 消耗物品的id和物品的数量
			...
			objSwf.infopanel.learnUpPanel.lvluppanel.btnItem2.htmlLabel = StrConfig['magicskill4'] .. string.format( StrConfig["magicskill2"], needNum[1],num1);   ---红色, 升Cấp条件不足
...
function UIXinfaSkillBasic:GetItemVo(IsPassSkill,skillId,learn,level)   
	local list = SkillUtil:GetLvlUpCondition(skillId,learn,level,IsPassSkill);     ----学习 learn = true || 升Cấp  shengji = false ,IsPassSkill 是用来区别绝学和被动技能
	local itemNameStr = {};
	local itemNeedNum = {};
	local itemList = {};
	local itemIcon = {};
...
function UIXinfaSkillBasic:OnSkillLearn(skillId)
	...
	for i,vo in ipairs(self.skilllist) do
		if vo.skillId == skillId then
			vo.lvl = 1; 
			vo.lvlStr =math.floor(math.floor(skillId % 10000000) % 1000 ).."Cấp1trọng";
			local cfg = t_passiveskill[skillId];
			if not cfg then
				vo.name ="" 
			else
...
function UIXinfaSkillBasic:OnMagicSkillLvlUp(skillId,lv)
	...
	for i,vo in ipairs(self.skilllist) do
		if math.floor(math.floor(vo.skillId / 1000000 ) * 100 + math.floor(vo.skillId % 1000000) / 1000 ) == math.floor(math.floor(skillId / 1000000 ) * 100 + math.floor(skillId % 1000000) / 1000 ) then
			vo.skillId = skillId;
			vo.lvl = lv;
			vo.lvlStr =math.floor(math.floor(skillId % 10000000) % 1000 ).."Cấp"..lv.."trọng"; 
			local cfg = t_passiveskill[skillId];
			if not cfg then
				vo.name ="" 
			else
