...
UIMagicSkillBasic.currSkillLvl = 0;	  ---当前技能等Cấp
--技能列表
...
function UIMagicSkillBasic:OnLoaded(objSwf)
	...
	objSwf.infopanel.learnUpPanel.lvluppanel.btnLvlUp.click = function() self:OnBtnLvlUpClick(); end  ---升Cấp技能
...
function UIMagicSkillBasic:OnLvlUprollOver(index)
	...
	local list = {};
	--print("鼠标悬浮的当前技能等Cấp:",self.currSkillLvl)
...
function UIMagicSkillBasic:ShowSkillList(isselectone)
	...
	if isselectone then   ---默认第一个
		...
		self:ShowRight(list[1].skillId,list[1].lvl);  ----显示第一个绝学技能的等Cấp的详情
	else
...
function UIMagicSkillBasic:GetSkillListVO(skillId,lvl,skillindex)
	...
	vo.lvl = lvl;
	vo.lvlUrl = "";
	vo.lvlUrl = ResUtil:GetMagicSkillIcon("level_"..lvl);      ----等Cấp阶数
	...
	vo.lvlStr = cfg.level.."Cấp"..lvl.."trọng";
	local level;
	if skillId < 1000000000 then
		if vo.lvl > 0 then  ---更具等Cấp获取不同的资源
			...
			vo.state = MountUtil:GetCanLvlUp(skillId,false,lvl);   --- 是否可以升Cấp
...
function UIMagicSkillBasic:ShowRight(skillId,lvl)
	...
	if cfg then
		if skillId < 1000000000 then
			...
			objSwf.infopanel.tfLvl.htmlText = string.format(StrConfig['magicskill001'],level,lvl);   ---%s/%strọng
		end
...
function UIMagicSkillBasic:ShowAttrs(strattr)
	...
	local nextAttrList = split(nextAttr,"#"); --下一Cấp的属性list
	for i,attrStr in ipairs(attrList) do
		local attrvo = split(attrStr,",");
		local vo = {};
		vo.type = AttrParseUtil.AttMap[attrvo[1]];
...
function UIMagicSkillBasic:ShowLearnUpPanel()
	...
	elseif self.currSkillLvl < maxLvl then
		...
		if UIMagicSkillBasic:CanFeedItem(itemList[1].id,itemList[1].num) then   --- 消耗物品的id和物品的数量
			...
			objSwf.infopanel.learnUpPanel.lvluppanel.btnItem2.htmlLabel = StrConfig['magicskill4'] .. string.format( StrConfig["magicskill1"],num1);   ----升Cấp条件
			...
			objSwf.infopanel.learnUpPanel.lvluppanel.btnItem2.htmlLabel = StrConfig['magicskill4'] .. string.format( StrConfig["magicskill2"],num1);   ---红色, 升Cấp条件不足
...
function UIMagicSkillBasic:GetItemVo(skillId,learn,level)   
	local list = SkillUtil:GetLvlUpCondition(skillId,learn,level);     ----学习 learn = true || 升Cấp  shengji = false 
	local itemNameStr = {};
	local itemNeedNum = {};
	local itemList = {};
	local itemIcon = {};
...
function UIMagicSkillBasic:OnBtnLvlUpClick()
	-- print("---------------点击升Cấp按钮",self.currSkillId,SkillModel:GetSkill(self.currSkillId).lv)
	...
	if skillId < 1000000000 then
		for i,vo in ipairs(self.skilllist) do
			if vo.skillId == skillId then
				vo.lvl = 1; 
				local cfg = t_skill[skillId];
				vo.lvlStr = cfg.level.."Cấp 1 trọng";
				local cfg = t_skill[skillId];
				if not cfg then
					vo.name ="" 
				else
	...
	if skillId < 1000000000 then
		...
		for i,vo in ipairs(self.skilllist) do
			...
			if  gid == juexueGid then 
				...
				if not cfg then return; end
				-- debug.debug()
				vo.lvlStr = cfg.level.."Cấp"..lv.."trọng";
				local cfg = t_skill[skillId];
				if not cfg then
					vo.name ="" 
				else
...
function UIMagicSkillBasic:CheckList()
	local objSwf = self.objSwf;
	if not objSwf then return; end
	self.isCanLvUpNum = 0    --如果红点提示需要显示数字，记录可以升Cấp的数量
	for i,vo in ipairs(self.skilllist) do
	...
	-- print("有几个可以升Cấp呢:",self.isCanLvUpNum)
