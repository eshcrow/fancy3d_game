...
local desTitle = {"Ngày đầu tiên thông qua 360 nhân viên bảo vệ đăng nhập "," hai ngày đầu tiên thông qua đăng nhập 360 nhân viên bảo vệ "," người dùng trả tiền 3 ngày qua 360 nhân viên an ninh đăng nhập",""}
...
function wanChannelRewardView:OnLoaded(objSwf)
	...
	objSwf.pageOne.label = "360 Vệ sĩ Giáng Sinh dâng tặng lễ vật"
end
...
function wanChannelRewardView:OnGetRewardClick(target)
	...
	if playLv < canInLevel then
		FloatManager:AddNormal("Cấp bậc chưa đủ")
		return
	end
	--点击间隔
	if GetCurTime() - self.lastSendTime < 3000 then
		FloatManager:AddNormal("3 Giây sau thử")
		return
	end
